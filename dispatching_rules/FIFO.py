...
from MTWR import MTWR_worker
from config import default_config
from sklearn.decomposition import PCA
import os

from dispatching_rules.JSSEnv import JssEnv
...
def pars_sol(jobs, operations, instance_name):
    solution_EST = np.zeros((jobs, operations), dtype=int)
    f = open('..' + os.sep + 'est_solution' + os.sep + instance_name + '.lp', 'r')
    main_text = f.read()
    f.close()
...
def write_in_file(jobs, operations, features, cluster_assignment, counter, instance_name):
    clust = instance_name + '({}).lp'.format(counter+1)
    path = '..' + os.sep + 'assign_to_clus' + os.sep + clust
    f = open(path, 'w')
    for i in range(operations * jobs):
...
if __name__ == "__main__":
    ...
    head_tail = os.path.split(instance_path)
    instance_name = head_tail[1]
    ...
    solution_EST = pars_sol(jobs, operations, instance_name)
    ...
    while(counter < 16):
        ...
        write_in_file(jobs, operations, features, cluster_assignment, counter, instance_name)
